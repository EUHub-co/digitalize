    ...
    return (
        <section id="capabilities" className="py-24 bg-[var(--background)]">
            <div className="container mx-auto px-4">
                ...
                {/* Bento Box Grid */}
                {/* minmax row sizing: md keeps the 250px rhythm (demo spans 3 rows full-height),
                    but rows may grow to content — on mobile the demo card (header + chat +
                    CTA footer) far exceeds one 250px row and would be clipped by overflow-hidden. */}
                <div className="group grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[minmax(250px,auto)] relative z-10">
                    ...
                    <GlassCard className="col-span-1 md:col-span-2 md:row-span-3 flex flex-col justify-between border border-[#E2E8F0] dark:border-[var(--card-border)] bg-[var(--card-bg)] backdrop-blur-md transition-all duration-300 opacity-100 group-hover:[&:not(:hover)]:opacity-50 dark:group-hover:[&:not(:hover)]:opacity-30 hover:scale-[1.02] hover:-translate-y-2 shadow-[0_10px_30px_rgba(0,0,0,0.08)] dark:shadow-none hover:shadow-[0_15px_40px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_0_40px_rgba(0,229,255,0.2)] overflow-hidden relative">
                        ...
                        {/* Conversion CTA — fixed-height footer under the chat panel; its top border
                            closes the open-bottomed chat window. Primary action stays on-site (#contact);
                            the FAIA/LKW-Control attribution is demoted to a muted footnote below it. */}
                        <div className="mx-8 flex-shrink-0 border-t border-slate-200 dark:border-[#2A2D3E] pt-4">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <p className="text-sm font-semibold leading-snug">
                                    {dict.features?.demo?.ctaText || 'Want an agent team like this running your operation?'}
                                </p>
                                <a href="#contact" className="btn btn-primary !text-sm !px-5 !py-2 flex-shrink-0">
                                    {dict.features?.demo?.ctaButton || 'Get the free audit'}
                                </a>
                            </div>
