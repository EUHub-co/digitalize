...
>
> **Not done:** Task 10 (domain cutover — user is doing `infra.` + `deploy.` mappings themselves; both currently point nowhere / `deploy.` still on old `euhub-ai-web`), Phases 5–6 (strip `ai-studio`, docs/memory wrap-up — **must wait until the user confirms both domain mappings are live and verified**, per the plan's core safety rule). Resume: wait for user confirmation of D10 cutover, then Task 11.
